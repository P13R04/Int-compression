...
public final class BenchCLI {
    ...
    private static final class Row {
        ...
        final Stats comp;
        final Stats decomp;
        Row(CompressionType t, int w, Stats c, Stats d){ this.type=t; this.words=w; this.comp=c; this.decomp=d; }
    }
    ...
    private static Row benchVariant(CompressionType type, int[] data, int runs) {
        ...
        return new Row(type, words, summarize(compMs), summarize(decompMs));
    }
    ...
    private static void printAsciiTable(List<Row> rows, Appendable out) throws IOException {
        String[] headers = {"Variant", "words", "comp_med(ms)", "comp_IQR", "decomp_med(ms)", "decomp_IQR"};
        List<String[]> lines = new ArrayList<>();
        lines.add(headers);
        for (Row r : rows) {
            lines.add(new String[]{
                r.type.name(), String.valueOf(r.words),
    ...
    private static void writeCsv(List<Row> rows, Path out) throws IOException {
        try (BufferedWriter w = Files.newBufferedWriter(out)) {
            w.write("variant,words,comp_median_ms,comp_iqr_ms,decomp_median_ms,decomp_iqr_ms\n");
            for (Row r : rows) {
                w.write(String.join(",",
                        r.type.name(),
